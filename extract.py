...
def extract_teacher_button(api_key: str, text: str) -> dict:
    ...
    # """
    prompt = f"""
    你是一个数据收集助手，负责分析网页HTML源码，识别并返回最可能引导至高校学院教师信息页面的链接。

    需要分析的HTML源码：
    {text}

    请按照以下判断标准依次评估每个链接（优先级从高到低）：
    1. **链接文本明确包含关键词**：如"教师队伍"、"师资力量"、"Faculty"、"Professors"、"教学团队"等。
    2. **链接文本包含教师相关词汇**：如"教师"、"老师"、"导师"等。
    3. **URL路径中包含关键词**：如"teacher"、"faculty"、"szdw"等。

    附加规则：
    - 如果存在多个符合上述标准的链接，优先选择层级更深、内容更具体的页面（例如：子页面优于父页面）。具体示例如下：
    - 若HTML中包含：
    ...
    - 则应返回更具体的子链接（如"专任教师"），而非父链接（如"师资队伍"）。

    请严格遵循以下JSON格式返回结果，无需任何额外说明：
    {{
        "button_text": "按钮文本",
        "url": "对应的URL"
    }}
