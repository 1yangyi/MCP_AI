...
try:
    ...
    # 如果UTF-8失败，尝试其他编码
    with open('html.txt', 'r', encoding='latin-1') as file:
        text_content = file.read()
all_norm = extract_and_normalize(text_content)
for item in all_norm:
    if item["URL"] is None:
        item["URL"] = ""
    if item["URL"] is not None:
        if not item["URL"].startswith("http"):
            item["URL"] = urljoin("https://chemistry.fudan.edu.cn/45141/list.htm", item["URL"])
print(json.dumps(all_norm, ensure_ascii=False, indent=2))
