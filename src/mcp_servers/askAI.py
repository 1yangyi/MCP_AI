...
{text}
请以字符串格式直接返回该学校所有二级学院所在的位置（按钮）的文本。例如："学院概况"、"院系设置"。
注意：仅输出一个最可能的按钮文本，不要附加解释。
例如，给定示例：{example}，输出应为："院系设置"。
"""
...
if __name__ == "__main__":
    ...
    if not DEEPSEEK_API_KEY:
        ...
        try:
            ...
            text_obj = read_json_file("hzkjdx.json")
            ...
            if result.get("status") == "success":
                print("识别结果(按钮文本):", result.get("entities"))
            else:
