...
import glob
from extract import check_next_page,check_similar_page,extract_teachers_with_deepseek
    ...
    for school in schools_data:
        ...
        for college in colleges:
            ...
            if not college_name in error_colleges:
                continue
            if college_url != '':
                not_empty_colleges.append(college['name'])
                err_len += 1
        if err_len == 0:
            continue
        print(f"需要处理 {university_name}: {not_empty_colleges}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=err_len) as executor:
            ...
            for college in colleges:
                college_name = college["name"]
                college_url = college["URL"]
                if not college_name in not_empty_colleges:
                    continue
                if college_url == '':
                    continue
